...
Windows 10, Node 22, npm 11 locally (CI's Node 22 ships npm 10, and `package-lock.json` must stay valid for BOTH: regenerate it with `npx npm@10.9.3 install --package-lock-only --ignore-scripts`; npm only; there is no `yarn.lock`). Git identity for this repo is `foGledenalzi` with the GitHub no-reply email `333976650+foGledenalzi@users.noreply.github.com` (repo-local config, which is NOT cloned: set it in every fresh clone; GitHub rejects pushes that expose a private email, error GH007; never use a machine's global identity). Remotes: `origin` = `https://github.com/foGledenalzi/CCRUGen.git`, `upstream` = `lumpenspace/ccru` with its push URL disabled. Pushes happen only when the user explicitly asks (so far: the initial push, then the README and the handoff, then the Phase 1 completion commit 52fc8df, then the declutter, lockfile fix and notes up to 45a8662 (2026-09-26), then the Phase 2 context and planning docs plus a README refresh (2026-09-26); each was authorized by the user in words such as "push after phase completion", so later pushes still need a fresh OK unless the user says it is standing). **Never `git push` on your own initiative (any remote, any branch)**; pushing is user-approved and separate from all plans. Commits end with `Co-Authored-By: Claude Sonnet 5 <noreply@anthropic.com>`.

## Resuming in a fresh or cloud session

- **Read first:** `.planning/STATE.md` (position and decisions), `.planning/ROADMAP.md`, and the open todos in `.planning/todos/pending/` (002 = deferred Phase 1 review findings, 003 = dropped row clicks and mobile overlap; 001, the UI declutter, is done in `.planning/todos/completed/`). Notes live in `.planning/notes/` (for example the CI lockfile note). `/gsd-progress` re-orients. There is no pause handoff file at the moment; `/gsd-pause-work` writes one when needed.
- **Where we are:** Phase 1 (Foundations and Safety Net) is executed and independently verified (`.planning/phases/01-foundations-and-safety-net/01-VERIFICATION.md`, passed 15/15): frozen base-10 oracle, static export, engine boundary guard, licensing and repo guard. The UI declutter (CRT overlay, glitch effects, intro splash, panel clutter, no functionality removed) is also done. Phase 2 (Engine Core and Base-10 Migration) is discussed (decisions D-01..D-16 in `02-CONTEXT.md`), planned (13 plans in 10 waves, plan checker passed) and ready to execute: next is `/gsd-execute-phase 2`. It is the largest phase so far (each of the five data-source swaps runs the ~3 minute full gate), so pace it with `--wave N`.
