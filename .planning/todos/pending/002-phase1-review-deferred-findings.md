...
Track the Phase 1 review findings that were consciously not fixed before closing Phase 1. Full detail, failure scenarios and suggested fixes are in `.planning/phases/01-foundations-and-safety-net/01-REVIEW.md`. Fixed before close: WR-02, WR-03, WR-04, IN-07.

## Deferred, with where each belongs

- **WR-01** (double base path on the header-title click under a sub-path): the route-transition provider that causes it is removed by todo 001 (UI declutter). Add the e2e click case from the review to `static-export.spec.ts` in that task.
